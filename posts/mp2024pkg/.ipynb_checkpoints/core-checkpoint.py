...
def show_dict(dct):
    rprint("Dictionary Overview:")
    rprint(f"Total keys: {len(dct.keys())}")
    rprint(f"Keys: {list(dct.keys())}\n")
    
    for i, (k, v) in enumerate(dct.items()):
        rprint(f"{i+1}. Key: '{k}'")
        rprint(f"   - Type: {type(v).__name__}")

        # 길이 확인이 가능한 타입인 경우 길이 정보 출력
        if hasattr(v, "__len__"):
            rprint(f"   - Length: {len(v)}")
        ...
        # 값 출력
        rprint(f"   - Values: {display_values}")
